document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderSummary() {
    ...
    document.getElementById("matchMetrics").innerHTML = `
      <div class="operator-metric"><span>Selesai</span><strong>${completed}/${matches.length}</strong></div>
      <div class="operator-metric"><span>Menunggu</span><strong>${pending}</strong></div>
      <div class="operator-metric"><span>Court</span><strong>${current.setup.courtCount}</strong></div>
    ...
    document.getElementById("fairnessLabel").textContent =
      `${fairness.score}% · ${fairness.min}-${fairness.max} game sejak R${fairness.balanceEpochRound}`;
  }
    ...
    if (!items.length) {
      chooser.innerHTML = `
        <div class="compact-empty">
          ...
          <span>Tambahkan match baru jika pertandingan masih berlanjut.</span>
        </div>`;
      return;
    }
  ...
  function renderRounds() {
    ...
    if (!rounds.length) {
      roundsContainer.innerHTML = `
        <div class="compact-empty">
          <strong>${current.rounds.length ? "Tidak ada hasil yang cocok" : "Belum ada pertandingan"}</strong>
          <span>${current.rounds.length
            ? "Ubah filter atau kata pencarian."
            : "Tambahkan match pertama lewat tombol Tambah Match."}</span>
        </div>`;
      return;
    }
